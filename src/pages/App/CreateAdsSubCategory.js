...
import {  getMarketPlaceSubCategories } from '../../util/Action';
import Back_Arrow from './../../images/Back_Arrow.svg';
const { width, height } = Dimensions.get('window');

export default class CreateAdsSubCategory extends Component {
    ...
    componentDidMount() {
        ...
        this.getMarketPlaceSubCategoriesAction();
    }
    async getMarketPlaceSubCategoriesAction() {
        const category_id = this.props.navigation.getParam("category_id");
        let customer_id = await AsyncStorage.getItem('userid');
        let Token = await AsyncStorage.getItem('token');
        const dataForGetMarketplaceSubCategories = {
            access_token: Token,
            current_user_id: customer_id,
            category_id: category_id
        }
        this.setState({ loader: true })
        getMarketPlaceSubCategories(dataForGetMarketplaceSubCategories).then((responseJson) => {
    ...
    renderItem = ({ item }) => {
        ...
        return (
            ...
            <TouchableOpacity onPress={() => navigate("MarketPlaceCreateAdvert", {  sub_category_id: item.sub_category_id, category_id:this.state.category_id })}>
                
                <Row style={{ height: 60, paddingHorizontal: 20 }}>
                    <Col style={{ justifyContent: 'center', width: "90%" }}>
                        <Text numberOfLines={1} style={{ color: "#333", fontSize: 25, fontFamily: "Roboto-Medium" }}>{item.category_name}</Text>
                    </Col>
                </Row>
                <LinearGradient
    ...
    render() {
        const { navigate } = this.props.navigation;
        const {loader, category_id, category_name} = this.state;
        if (!loader) {
            return (
                <View style={{ flex: 1, backgroundColor: '#fff' }}>
                    {/* <Row style={{ marginTop: -20, height: 120 }}>
                        ...
                        <Row style={styles.slide2Logo}>
                            <TouchableOpacity style={{ alignItems: 'center', padding: 20, width: 60, }} onPress={() => navigate("MarketPlaceAdsCategory")}>
                                <Col style={{ justifyContent: 'center' }}>
                                    <Back_Arrow width={15} height={15} />
                                </Col>
                            </TouchableOpacity>
                            <Col style={styles.header}>
                                <Text style={{ color: "#fff", fontSize: 17, fontFamily: 'Roboto-Bold'  }}>{t("Categories")}</Text>
                            </Col>
                        </Row>
                    </Row> */}
                    ...
                    <Row style={styles.headerContainer}>
                        <View style={styles.top_background}>
                            <View style={styles.top_content}>
                                <TouchableOpacity style={styles.headerBackArrow} onPress={() => navigate("MarketPlaceAdsCategory")}>
                                    <Col style={{ justifyContent: 'center' }}>
                                        <Back_Arrow width={15} height={15} />
                                    </Col>
                                </TouchableOpacity>
                    ...
                    <Row style={{ height: 50, marginHorizontal: 20 }}>
                        <Col style={{ justifyContent: 'center', alignItems: 'center' }}>
                            <Text numberOfLines={1} style={{ color: "#333", fontSize: 25, fontFamily: "Roboto-Medium", color: "#62BB46" }}>{t('Select Subcategory')}</Text>
                        </Col>
